...
function renderHomeRows(rows) {
  ...
  elements.assetList.innerHTML = rows.map((row, index) => {
    ...
    return `
      <button class="asset-row" type="button" data-index="${index}">
        <span class="asset-title">${(state.page - 1) * state.pageSize + index + 1}. ${escapeHtml(asset.name)}</span>
        <span class="asset-meta">
          <span class="${toneByBias(structure)}">${escapeHtml(structure)}</span>
          <span class="dot">•</span>
          <span class="${toneByBias(action)}">${escapeHtml(action)}</span>
          <span class="dot">•</span>
          <span class="${toneByConfidence(confidence)}">${confidence}%</span>
          <span class="dot">•</span>
          <span class="${ideal === "Sudah" ? "tone-green" : "tone-red"}">${ideal}</span>
        </span>
      </button>
    `;
  }).join("");
...
function renderAnalysis(payload) {
  const { asset, close, change, analysis, forecast } = payload;
  const currency = asset.currency || "USD";
