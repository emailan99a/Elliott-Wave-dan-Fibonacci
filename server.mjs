...
function cryptoPerpSymbol(asset) {
...
async function getCoinGeckoHistory(asset, safeDays) {
  ...
  if (!output.length) throw new Error(`No CoinGecko daily history for ${asset.symbol}`);
...
async function handleApi(req, res, url) {
  ...
  if (path === "/api/history") {
    ...
    const result = withPreviousAnalysis(asset, candles, threshold);
    json(res, 200, {
      asset,
      candles,
      analysis: result.analysis,
